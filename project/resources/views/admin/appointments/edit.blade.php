        ...
        <div class="card shadow">
            <div class="card-header">
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800">{{ __('edit subsctiption')}}</h1>
            ...
            <div class="card-body">
                <form action="{{ route('admin.appointments.update', $appointment->id) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="employee_id">
                            @foreach($employees as $id => $employee)
                                <option {{ $id == $appointment->employee->id ? 'selected' : null }} value="{{ $id }}"> {{ $employee }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select class="form-control" name="client_id" >
                            @foreach($clients as $id => $client)
                                <option {{ $id == $appointment->client->id ? 'selected' : null }} value="{{ $id }}"> {{ $client }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control datetimepicker" id="finish_time" name="finish_time" value="{{ old('finish_time', $appointment->finish_time) }}" />
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="comments" placeholder="{{ __('comments') }}" name="comments" >
                            {{ old('comments', $appointment->comments) }}
                        </textarea>
                    </div>
                    <div class="form-group">
